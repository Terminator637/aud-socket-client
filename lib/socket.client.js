...
module.exports.client = function(namespace, config) {
        ...
        var real_connect = function(index) {
            ...
            socket.on('connect', function() {
                ...
                socket.on('error', function(err) {
                    for (var i in queue) {
                        for (var n in queue[i][0])
                            setImmediate(queue[i][1], err, null);
                        delete queue[i];
                    }
                });
                ...
                socket.on('line', function(data) {
                    ...
                    if (json[0] && queue[json[0]]) {
                        var queue_item = queue[json[0]];
                        queue_item[1](json[1], json[2]);
                        if (0>=--queue_item[0])
                            delete queue[json[0]];
                    }
                });
            ...
            socket.on('close', function() {
                if (self._state === 2) {
                    ...
                    for (var i in queue) {
                        for (var n in queue[i][0])
                            setImmediate(queue[i][1], new Error('Server disconnected'), null);
                        delete queue[i];
                    }
    ...
    Client.prototype.send = function(method, args, callback) {
        ...
        if (data) {
            ...
            if (callback) this.queue[this.reqno] = [1, callback];
    ...
    Client.prototype.sendAll = function(method, args, callback) {
        ...
        if (data) {
            if (callback) this.queue[this.reqno] = [this.sockets.length, callback || this.doNothing];
            for (var i = 0; i < this.sockets.length; i++) {
